...
\begin{center}
  {\Large \textbf{185.A05 Advanced Functional Programming SS 24}}  \\ [1ex] 
          ...
          \textbf{Submission deadline:} Monday, 10th June 2023, noon (no continuous update)  \\ [0.5ex]
          \textbf{Contact:} prof. Jens Knoop, jens.knoop@tuwien.ac.at, \\
                            Francesco Pontiggia, francesco.pontiggia@tuwien.ac.at \\
                            Dominik Leko, dominik.leko@tuwien.ac.at \\
                            Tim Dirr, tim.dirr@tuwien.ac.at
...
Implementing this core awards up to 275 points.
...
\begin{itemize}
    ...
    \item $\{ \exists \llglob \big(\text{ select } \implies  (\forall \lnext \text{ soda })\big) \}$ := 
    it exists a path such that, every time a selection is encountered, all paths starting from it release a soda. 
            This does not hold.
\end{itemize}

\clearpage
\section{Extension 1 - Parsing MINI$^{--}$ Programs (125 P)}
\label{sec:ext1}
...
In contrast to common programming languages, \code{return} statements in MINI$^{--}$ 
are syntactically restricted to return the value of a single variable instead of a general expression.
...
which is already there in the type of TS we are using.
...
We define now the semantics of a MINI$^{--}$ program $P$ in terms of a Transition System, 
...
Formally, a state is given by $\langle P, \mathbf{\gamma} \rangle$, where program $P$ is just 
...
of initial boolean values for the set of arguments of the MINI$^{--}$ program $P$. 
This can be represented in set notation by $2^{arg(P)}$, where $arg$ is the set of arguments of $P$. 
...
where the symbol $\downarrow$ indicates a state where there is no more program to evaluate.
\begin{align*}
    &\; \frac{\varphi \notin \gamma}{\langle \text{ if }(\varphi)\text{\{B\}; P}, \mathbf{\gamma} \rangle \rightarrow \lightning} &&\;
      \frac{\varphi \notin \gamma}{\langle \text{ if }(\varphi)\text{\{B\} else \{E\}; P}, \mathbf{\gamma} \rangle \rightarrow \lightning} \\ \\
    ...
    &\;\frac{\mathbf{\gamma} \vDash (Expr \mapsto v)}{\langle x:=Expr;P , \, \mathbf{\gamma} \rangle \rightarrow \langle P,\mathbf{\gamma}[x \mapsto v] \rangle } &&\;
      \frac{Expr \notin \gamma}{\langle x:=Expr;P , \, \mathbf{\gamma} \rangle \rightarrow \lightning } \\  \\
    &\;\frac{Expr \in \gamma}{\langle print\_bool(expr);P , \mathbf{\gamma}\rangle \rightarrow \langle P, \mathbf{\gamma} \rangle} &&\;
       \frac{Expr \notin \gamma}{\langle print\_bool(Expr);P , \mathbf{\gamma}\rangle \rightarrow \lightning} \\  \\
    &\;\frac{x \notin \gamma}{\langle x=read\_bool;P , \mathbf{\gamma} \rangle \rightarrow \lightning} &&\; \\ \\
    &\;\frac{x \in \gamma}{\langle x=read\_bool;P , \mathbf{\gamma} \rangle \rightarrow \langle P,\mathbf{\gamma}[x \mapsto T] \rangle} &&\;
       \frac{x \in \gamma}{\langle x=read\_bool;P , \mathbf{\gamma} \rangle \rightarrow \langle P,\mathbf{\gamma}[x \mapsto F] \rangle }\\ \\
    &\;\frac{x \in \gamma}{\langle return \text{ } x; \mathbf{\gamma} \rangle \rightarrow \langle \downarrow, \mathbf{\gamma} \rangle} &&\;
       \frac{x \notin \gamma}{\langle return \text{ } x; \mathbf{\gamma} \rangle \rightarrow \lightning} \\ \\
...
We use the notation $\mathbf{\gamma}[x \mapsto v]$ to denote the state that is obtained 
when updating the value of the variable $x$ to $v$. 

A statement is invalid if it uses a variable $x$ that has not been defined before.
We use notation $x \notin \gamma$ to indicate that variable $x$ has no binding in 
the current state. With a slight abuse of notation, we use $Expr \notin \gamma$ or $\varphi \notin \gamma$
to indicate that a variable with no binding occurs in $Expr$ or $\varphi$. An invalid statement makes the
...
\begin{figure}[H]
    \begin{center}
        \includegraphics[scale = 0.20]{pictures/model.png}
    \end{center}
  \end{figure}

\clearpage

\section{Extension 2 - Bounded LTL Model Checking (125 P)}
\label{sec:ext2}
With this extension, you are required to extend the verification power of MiniCheck 
targeting a different class of properties, 
...
Differently with respect to CTL, LTL embraces a \emph{linear} view of the time.
The interpretation of a LTL formula $\varphi$ over a state $s$ requires that $\varphi$ 
is satisfied by \textbf{all} possible computations (paths) that start in $s$. There are no 
path quantifiers $\exists$ or $\forall$.

The full LTL algorithm requires some more involved formalisms, hence we will not deal with it. 
Instead, we will implement a \textbf{bounded} model checking algorithm. 
To do so, we enumerate all possible paths that start from initial states with size up to a certain size $k$ 
(where the bound $k$ is provided by the user), and verify only these paths against the input formula. 
This means that the overall procedure is not complete, i.e. we are not formally guaranteeing that the formula holds for the TS at hand. 
However, this approach has some advantages: it is simple to apply and it is much more scalable. 

As modelling formalism, we will use the same as the core modules, 
i.e. Transition Systems without terminal states, and where Actions are irrelevant.

In the following, we will provide some information about LTL, 
and then present the tasks to be implemented with this extension.
...
its trace is just the induced infinite sequence of atomic propositions $ \sigma = L(s_0)L(s_1) \dots$.
Let $\sigma = A_0A_1A_2 \dots$ be an infinite sequence of labels. We use notation $\sigma [i \dots j]$
to indicate the subsequence $A_i \dots A_j$. Then $\vDash$ is defined by:
\begin{align*}
    ...
    &\; \sigma \vDash \lnext{\varphi} \text{ iff } \sigma [1 \dots] (= A_1 \dots) \vDash \varphi \\
...
\subsection*{A note for those interested}
CTL and LTL in general are \emph{uncomparable}, which means that there are some CTL formulae that
cannot be expressed in LTL, and viceversa. However, after having fixed 
a finite bound $k$, our bounded version LTL$_{k}$ could be entirely expressed in CTL, and hence this 
extension is not adding any expressivity to the tool.
