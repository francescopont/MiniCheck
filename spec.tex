...
Besides this, some previous knowledge in related fields helps and makes the project smaller, 
...
In the following, we provide some informal information about the type the formalisms we will use, 
...
\end{itemize}
They are used to specify how the system evolves from one state to another. The labelling function 
...
Note that, for simplicity sake, we consider only TS with no terminal states.

\subsubsection{Resources}
\cite[Paragraph 2.1.1]{BaKa}
\subsubsection{Task}
Define a suitable plain-text representation of TS,  a data type to 
represent them and implementing the corresponding parsing function. In particular,
take care of making sure that there are no terminal states, and that there is at least an initial state.

\todo{Add to the helper function the parsing of only this, and giving back results in case of failure}
...
\item On \code{--sat}, the input code should \todo{write me!}.
