...
%\todo[inline]{align this definition}
\begin{eqnarray*}
    & Sat(\text{true})          & := S \\
...
\begin{itemize}
    ...
    \item $\{ \exists \llglob \big(\text{ select } \implies  (\forall \lnext \text{ soda })\big) \}$ := it exists a path such that, every time a selection is encountered, all paths starting from it release a soda. This does not hold because of Tuwel Discussion ''MiniCheck Aufgabenstellung''. Thanks to Jan M\"uller for pointing this out.
\end{itemize}
...
\subsection{Examples}
%\todo[inline]{This examples do not reflect the syntax of MINI-- because of the the presence of integer values}
%\todo[inline]{Add the corresponding transition systems of the examples would be extremely beneficial}
